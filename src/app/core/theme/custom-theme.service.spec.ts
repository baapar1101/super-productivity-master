...
describe('CustomThemeService', () => {
  ...
  describe('migrateLegacyCustomTheme', () => {
    ...
    it('is a no-op for unknown legacy ids (allowlist enforced)', async () => {
      ...
      expect(localStorage.getItem(LS.CUSTOM_THEME)).toBeNull();
      // Unchanged means the untouched first-run pick, not the `default` ref.
      expect(service.activeRef()).toEqual({ kind: 'builtin', id: 'huma' });
    });
  ...
  describe('pickInitialActiveRef', () => {
    ...
    it('falls back to the default theme when the stored value is malformed', () => {
      // A malformed string still counts as "user has a stored value", so it
      // goes through `parseRef`, which yields the neutral `default` ref
      // rather than the first-run pick.
    ...
    it('does not write the first-run default to LS — leaves it for legacy migration', () => {
      // `migrateLegacyCustomTheme` short-circuits when LS already has a
      // value. Persisting on first run would lock new installs out of
